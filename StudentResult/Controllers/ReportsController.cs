    ...
    public class ReportsController : Controller
    {
        ...
        private void LoadAttendanceDistribution(DbConnection conn, ReportsDashboardVM vm)
        {
            const string sql = @"
                ...
                FROM (
                    ...
                    FROM (
                        SELECT STUDENTID,
                               100 * SUM(CASE WHEN STATUS = 'Present' THEN 1 ELSE 0 END) / COUNT(*) AS pct
                        FROM SR.SR_ATTENDANCE
                        GROUP BY STUDENTID
                    )
                )
                GROUP BY bucket
        ...
        private void LoadCoursePassFail(DbConnection conn, ReportsDashboardVM vm)
        {
            const string sql = @"
                ...
                FROM SR.SR_MARKS m
                JOIN SR.SR_SUBJECTS su ON su.SUBJECTID = m.SUBJECTID
            ...
            const string sql = @"
                ...
                FROM (
                    SELECT CASE
                           ...
                           pct
                    FROM (
                        SELECT (MARKSSCORED / TOTALMARKS) * 100 AS pct
                        FROM SR.SR_MARKS
                        WHERE TOTALMARKS > 0
                    )
                )
        ...
        private void LoadAttendanceVsResult(DbConnection conn, ReportsDashboardVM vm)
        {
            const string sql = @"
                ...
                FROM SR.SR_STUDENTS s
                JOIN (
                    SELECT STUDENTID,
                           100 * SUM(CASE WHEN STATUS = 'Present' THEN 1 ELSE 0 END) / COUNT(*) AS avg_att
                    FROM SR.SR_ATTENDANCE
                    GROUP BY STUDENTID
                ) att ON att.STUDENTID = s.STUDENTID
                JOIN (
                    SELECT STUDENTID,
                           AVG(MARKSSCORED / TOTALMARKS * 100) AS avg_res
                    FROM SR.SR_MARKS
                    WHERE TOTALMARKS > 0
                    GROUP BY STUDENTID
